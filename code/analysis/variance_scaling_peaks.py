...
def main():
	...
	fisher = FisherAnalysis.read("../../data/fisher_peaks.pkl")
	...
	with Database("../../data/variance_scaling_expected_peaks.sqlite") as db:
		...
		true_covariance_ensemble = Ensemble(np.load("../../Om0.260_Ol0.740_w-1.000_ns0.960_si0.800/512b240/Maps200/peaks_s0.npy"),columns=feature_columns).cov()
		...
		for n in nsim:
			...
			ensemble_nsim = Ensemble(np.load("../../Om0.260_Ol0.740_w-1.000_ns0.960_si0.800/512b240/Maps{0}/peaks_s0.npy".format(n)),columns=feature_columns)

			#Bootstrap ensemble_sim and compute the parameter variance for each resample
			for nr in nreal:
				print("[+] Bootstraping peak constraints, nsim={0}, nreal={1} with {2} resamples".format(n,nr,resample))
