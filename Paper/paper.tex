...
%ZH: I changed to more catchy title:
%\title{Simulating sample variance in weak lensing: effects on parameter inferences}
%AP: Ok
...
%However, we find that in practice, a larger number of
%simulations might be needed to measure accurate feature means and to
%avoid parameter biases.
%ZH: I am not sure about the last sentence. I believe it is based on
%    an overly stringent reading of Fig.2, and that in fact we could
%    claim here, instead, that N_s=2 simulations are also sufficient
%    to get the correct mean.
%    See my comments at the end of the Conclusion section. 

%AP: Ok, sentence removed
...
%ZH: also cite Joop Schaye's hydro-simulation papers on lensing (Semboloni et al), and earlier papers by Martin White et al...  Lloyd Knox et al... and Zentner et al... (I think all of these are in Xiuyuan's paper).
%AP: done
...
%ZH: Add here some citations of other groups - Marian+Bernstein.... Bhuvnesh Jain et al... Refregier et al... Dietrichh+Hartmann... Shirasaki+Yoshida.. (again just copy from our previous papers).
%AP: done
and the  assessment of various systematics
\citep{MinkShirasaki,Sys-Bard,Sys-Chang,Sys-Huterer}.
%ZH: cite papers by Jan+Morgan+Bard... and also Chihway Chan...Shirasaki+Yoshida.. and older ones by Dragan Huterer on photo-z errors..
%AP: done
...
%ZH: I think 'cosmic variance' usually refers to observations of the actual sky (which is finite). 
%AP: OK 
...
%MM: also cite Taylor Joachimi and Kitching
%AP: done
...
multiple times.
%ZH: Add citations here of papers resorting to this technique
...
%ZH: ``mass resolution'' here means DM particle mass ?
%AP: fixed
...
%ZH: we should cite some paper here. We didn't invent most of this
%procedure ourselves (except for maybe the 3-times thicker simulation
%output, and the random mixing of the planes from different N-body sims ?).
%AP: I added the citation to Sato et. al, which refer to this procedure without citing anyone
...
%ZH: should we specify the range of the random shifts (0 to 1/2 the lens plane dimension?)
%AP: This is not necessary, since the shifts are periodic, in principle one could choose a random number of pixels. In particular this number should be between 0 and 4095, because each lens plane has 4096 pixels
...
%ZH: we should add 2 sentences on how you measured kappa.  Directly from the ray-tracing?
%   (but above, when you discuss ray-tracing, you mention only gamma).  Or did you do 
%    Kaiser-Squires on the gamma field ? 
%AP: done, we measure kappa directly from the ray tracing
...
%ZH: state here the range and binning choice of $\kappa_0$
%AP: done
...
%ZH: it would be logical to mention the other w,Sig,Om, sims here, and point to \ref{simtable})
%AP: done, added the below
...
%ZH: ``Hat'' here refers to an estimator?
%AP: Yes: quantities without the hat are the expectation values 
$\bb{d}(\bb{p})$ be the
%smooth expectation
%ZH: I believe ``smooth expectation'' means true mean value, yes?
%AP: yes
...
$\Sigma_\bb{p}=\bb{F}^{-1}$, we find the result~\footnote{The details of the calculation are given in Appendix A.}:
...
%ZH: we should probably cite Hartlap et al. and Taylor et al. here for this Psi bias.  (I assume this is what you mean in eq. 19)
%AP: done
...
%ZH: I am confused.  Why is a new symbol \sigma necessary here, on top of \Sigma ?
%    (And also: here you write \Sigma_pp, but above it was denoted \Sigma_p  )
%AP: fixed. I introduced the \sigma symbol for the single parameter variances (as opposed to the full parameter covariance), because these are the quantities we plot
...
%ZH: I added pathological here.  Do you agree?  It's only negative in case 1, when
%    N_p > N_b (which is impossible -- cannot measure N_p parameters with N_b observables)
%    or in case 2 (when N_b > N_p, so that's OK, but this case ust means we ``forgot'' to correct for the psi-bias).
%AP: agreed
...
\caption{PDF of the $\kappa$ power spectrum
  ...
  $N_r=1024$ realizations.
%ZH: I added the Nr=1000 above. Yes?
%AP: Yes, 1024 in reality, but it doesn't change much
  The dashed black curves correspond to ensembles generated with $N_s=1$ and
  $N_r=128000$. For $N_s\geq2$, the distributions appear
  similar to the eye; this similarity is confirmed by the comparisons
  in Figures \ref{means_ns} and \ref{ps_var} below.}
%MM: added last two sentences
%MM: Thick black line draws the eye and looks like the most significant thing on the plot. replace with dotted black line or something else that stands out less.
%ZH: Also, include a label ``N_r=10^5'' with the other N_s=... labels in the figure panels 
%   (it is just confusing why there are labels for all the other curves, but not for the thick black)
%AP: done
\label{ps_pdf}
\end{figure*}
...
% $d$  
%ZH: I think you mean  \bb{\bar{d}} here?  You introduced a lot of different
%    subtle notations in the previous section, so we should  follow those ....
%AP: OK
...
%ZH: I replace ``of'' with ``above''.  These are cumulative peak counts N(>kappa_0) ?
%    Or do you mean differential counts in some delta-kappa_0 bins?   Then you should
%    correct the text (and state the bin width).  Table II contains some kappa-bins
%    but no obvious relation to the 0.05, 0.17  and 0.28 mentioned here...
%AP: The peak counts are differential, I added the clarifications; I chose three values of kappa0 for the sake of presentation simplicity (one value in the low height peaks, one in the middle and one in the large)
...
%MM: added last sentence stating conclusion to be drawn from the figure
%ZH:  Good idea.  All figures captions should similarly state a conclusion!
%ZH:  What is the horizontal dashed line?  It is never mentioned in the caption or
%     the text.   I guess you decided to mark the ``0.1 sigma'' deviation?
%     We should state that.    I suppose this is what Morgan refers to by 
%    ``statistically indistinguishable'' ....  0.1 sigma.
%AP: Ok, added the clarifications to the caption
\label{means_ns}
\end{figure}
...
%ZH: It would be better to write the features on the legend embedded in the figure.
%    Why make the reader flip back-and-forth between the figure and the Table ?
%AP: done
  ...
  unless $N_r\gg N_b$.}
%MM: added last sentence stating conclusion to be drawn from the figure
%in legend box change line that reads Nb=7 divided by 8
%AP: done, the legend is comprehensive now
\label{curvingnb}
\end{figure}
...
%ZH: We need to explain what you mean by best-fit (presumably you treat ``D'' as a free parameter).
%   I added that. But if you meant something else, please correct.
%AP: OK
  The thick black curve corresponds to an ensemble of $N_r=128000$
  realizations from a single simulation ($N_s=1$).
%ZH: I added 'conclusion':
%AP: OK
...
%ZH: state here what N_r was actually used to approximate the infinity limit
%AP: the value plotted is actually $N_r\rightarrow\infty$, because it is estimated from the intercept of the fit. Even if all the points have Nr finite, the intercept of the line corresponds to Nr infinite. 
...
%ZH: Just to make sure: what you mean here is an averaging of sigma_infty over the 16 different
%    cases of N_s ?   
%AP: Yes
...
%ZH: What ell and kappa ranges were used?
%ZH: Added conclusion:
%AP: OK, added binning information
...
%ZH: added above 2-12.  OK ?
%AP: the actual numbers are 2-13, but again, only minor change
...
%
%ZH:  I changed ``l''  to ``\ell'' in many places, for the multipole
%AP: OK, I fixed this throughout the rest of the paper
\begin{equation}
\label{gaussianvar}
...
%ZH: previously you only stated ``power spectrum'' for Figure 5.  But I assume Fig.4 is
%    also based on the power spectrum?  If not, please correct.
%AP: OK, fixed
...
%ZH: I changed the wording above.  We didn't really prove that these are statistical errors,
%    and not some systematic shift to low power when N_s is low.
%AP: OK

Interestingly, we need as few as $N_s=2$ simulations to recover the
right PDF
%ZH: what is ``location parameter'' ?
%location parameter 
%AP: location parameter usually indicated where the PDF peaks. This might be the mean, but not always. 
...
($\kappa_0\approx0.3$).
%ZH: I think this text above is a bit misleading.  Wny would we actually impose the ``0.1 sigma''
%    requirement?    It sounds too stringent.  If we instead imposed, say, 0.5 sigma, would
%    N_s=2 always be sufficient?    We should probably at least add this (delete if untrue):
%AP: OK, agreed
...
%ZH: We could add more references here, e.g. the earlier Sato et al. 2009 paper, and the Takada and Spergel
%    papers looking at the non-Gaussian variance of the pow.spectrum.. 
%AP: Done: that is, in fact, the Sato 2009 paper, I mis-labelled it with Sato12 in the bibtex
...
%ZH: see my paraphrasing above.  I wanted to clearly distinguish the point
%    of this paragraph from the previous paragraph.
%AP: OK
...
%ZH: what is sufficiently large here?  I presume a ``few hundred'' 
%    (or $>100N_p$, based on eq. 13).  I added that.  Please correct if 
%    this is not OK.
%AP: OK, the quantity to compare to is Nb-Np though. If Nb=Np there is no degradation at every order
...
%ZH:  So I am not sure about this last conclusion.   
%     What exactly do you have in mind?   Is it Figure 2, which shows
%     some deviations above the horizontal dashed line, for N_s up to ~50 ?
%     But this is based on the super-stringent requirement that the bias
%     is <0.1*sigma.
%     The same figure seems to show that N_s=2 simulation is in fact
%     sufficient to measure the means to within 0.5*sigma, as well.
%     If you agree, we should revise this bullet to state this more
%     positive conclusions!
%AP: Agreed, I rephrased the bullet point
\end{itemize}
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%% APPENDIX %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%ZH: I did not carefully re-read the appendices.  I guess that is OK.
%    One cosmetic comment:  when you refer to equations, you need to say
%    ``as in eq. (xx)''  rather than  ``as in (xx)''.
%AP: done   
