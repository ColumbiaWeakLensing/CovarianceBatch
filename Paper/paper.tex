...
\end{equation}
%
\textbf{\textcolor{blue}{We chose $\bb{p}_0$ to be the triplet $(\Omega_m,w,\sigma_8)=(0.26,-1,0.8)$.}}
...
$\langle(\bbh{d}_{\rm obs}-\bb{d}_0)(\bbh{d}_{\rm obs}-\bb{d}_0)^T\rangle_O=\bb{C}$.  When we perform an observation
...
statistical fluctuations of $\bbh{\Psi}$. Denoting the \textbf{\textcolor{blue}{true}}
parameter covariance, i.e. the usual \textbf{\textcolor{blue}{inverse}} Fisher matrix, as
...
\end{widetext}
%
\textbf{\textcolor{blue}{Although we truncated the expansion to second order in $1/N_r$, an exact expression for $\langle\h{\Sigma}_\bb{p}\rangle$ has been proposed by \citep{Taylor14}}}
\begin{equation}
\label{empiricalexact}
\textcolor{blue}{\langle\h{\Sigma}_\bb{p}\rangle_{\rm empirical} = \Sigma_\bb{p}\left(\frac{N_r-2}{N_r-N_b+N_p-2}\right)}
\end{equation}
%
\textbf{\textcolor{blue}{This empirical expression reduces to equation (\ref{quarticdegradation}) when expanded at order $O(1/N_r^2)$ but, to our knowledge, no first principles proof of its correctness exists.}} 
...
\caption{Expectation value of the variance of $w$ computed from
  ...
  listed in Table~\ref{featuretable}.  The dashed and this solid curves
  show the analytic predictions from equation
  (\ref{quarticdegradation}) at orders $O(1/N_r)$ and $O(1/N_r^2)$,
  respectively. \textbf{\textcolor{blue}{The thick solid curves show the empirical predictions from equation (\ref{empiricalexact})}}. 
  The asymptotic variance $\sigma^2_{w,\infty}$ has been
...
\caption{The variance of $w$ in the limit of $N_r\rightarrow\infty$
  ...
  of $\sigma_{w,\infty}^2(N_s)$ in units of the mean over the \textbf{\textcolor{blue}{union of}} 16
  ensembles with different $N_s$ \textbf{\textcolor{blue}{and $N_r=1024$}}, for the power spectrum
...
be always sufficient. \textbf{\textcolor{blue}{As pointed out by \citep{NbodySample}, the box size used for the $N$--body simulations can also play an important role in the accuracy of the power spectrum ensemble means.}}
