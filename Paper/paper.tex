...
\end{equation}
%
We chose $\bb{p}_0$ to be the triplet $(\Omega_m,w,\sigma_8)=(0.26,-1,0.8)$.
...
statistical fluctuations of $\bbh{\Psi}$. Denoting the true
parameter covariance, i.e. the usual inverse Fisher matrix, as
...
\end{widetext}
%
Although we truncated the expansion to second order in $1/N_r$, an exact expression for $\langle\h{\Sigma}_\bb{p}\rangle$ has been proposed by \citep{Taylor14}
\begin{equation}
\label{empiricalexact}
\langle\h{\Sigma}_\bb{p}\rangle_{\rm empirical} = \Sigma_\bb{p}\left(\frac{N_r-2}{N_r-N_b+N_p-2}\right)
\end{equation}
%
This empirical expression reduces to equation (\ref{quarticdegradation}) when expanded at order $O(1/N_r^2)$ but, to our knowledge, no first principles proof of its correctness exists. 
...
\caption{Expectation value of the variance of $w$ computed from
  ...
  respectively. The thick solid curves show the empirical predictions from equation (\ref{empiricalexact}). 
...
\caption{The variance of $w$ in the limit of $N_r\rightarrow\infty$
  ...
  of $\sigma_{w,\infty}^2(N_s)$ in units of the mean over the union of 16
  ensembles with different $N_s$ and $N_r=1024$, for the power spectrum
...
be always sufficient. As pointed out by \citep{NbodySample}, the box size used for the $N$--body simulations can also play an important role in the accuracy of the power spectrum ensemble means.
