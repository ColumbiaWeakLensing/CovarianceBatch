...
%AP: done
%ZH2: The first row was a bit confusing, so I separated it into two rows. Also added words to the caption
...
%
%AP2: Technically the 1/Nr scaling has been obtained using the inverse Wishart distribution; if the data is non Gaussian, the covariance is not Wishart, but maybe one can say that for an arbitrary, not too crazy, distribution the leading term is still 1/Nr and the coefficient is not too different from Nb-Np... This is indeed what we find: the error degradation is basically the same should we use power spectra or peak counts which have different PDFs... I removed the mention to the Gaussianity, which is not what we test. What we test is eq. 10 in the low Nr limit and the (possible) deviations from 1/Nr when Nr is very large (due to the maps being pseudo independent). 
...
\caption{Bias in the variance of $w$,
  ...
  (eq.~\ref{covest}). The figure shows both the measured trends
  (solid lines) and their theoretical behavior according to eq.~(\ref{ourscaling}) with $D=N_b-N_p$. The asymptotic variance $\sigma^2_{w,\infty}$ has been estimated with a linear regression of $\langle\h{\sigma}^2_w\rangle$ vs $1/N_r$ with $500\leq N_r\leq 10000$ (black), with $\langle\h{\sigma}^2_w\rangle(N_r=50000)$ (red) and $\langle\h{\sigma}^2_w\rangle(N_r=100000)$ (green). No deviations from the expected $1/N_r$ behavior are observed until $N_r={\rm few}\times 10^4$.}
\label{wvar_nr}
\end{figure}
...
comparable to $N_r\sim {\rm few}\times10^4$. We can see from the plot that around these values of $N_r$ the $\langle\hat{\sigma}^2_w\rangle - \sigma^2_{w,\infty}$ curve becomes noisy and reaches negative values. This is a clear indication that the $1/N_r$ behavior is broken and a plateau is reached. The negative values in the plot are a consequence of the noise in the estimation of the $\langle\hat{\sigma}^2_w\rangle$ plateau value. We therefore conclude that a single $N$--body simulation is sufficient to construct an ensemble of $O(10^4)$ mutually independent convergence power spectra, before realizations start repeating themselves. For $N_r\gg10^4$, the shear realizations cannot be considered all independent.
...
%AP2: OK: Hartlap focuses on the Psi distribution, translating it into a parameter distribution using a naive Fisher analysis; what he does is in fact use the same simulation set for Psi and C, but this kind of discussion is absent from his paper.  
